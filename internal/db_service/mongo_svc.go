...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("EQUIPTRACK_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("EQUIPTRACK_API_MONGODB_PORT", "27017")
	...
	if svc.DbName == "" {
		svc.DbName = enviro("EQUIPTRACK_API_MONGODB_DATABASE", "xle-equiptrack")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("EQUIPTRACK_API_MONGODB_COLLECTION", "equipment")
	}
